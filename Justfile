...
#
# Order:
#   1. Build the C extension (so test_extension.php and test_e2e.php
#      have something to load).
#   2. Extension smoke test — exercises the recorder's normal entry
#      point (`-d extension=...`) and asserts the trace artefacts
#      land in the expected place.
#   3. End-to-end per-program tests — one program per universal-
...
#   4. RS-M7 request-span integration tests — a real `php -S` process and a
...
#      has, so a test file that is not listed here runs nowhere (see
#      `test-orphans` below).
#
# RS-M12 removed the pure-PHP `CodeTracerSpan` class and its
# `tests/test_span.php` unit suite.  That class wrote request metadata to a
# JSONL sidecar and never touched the container, so it could not be tested
# for the property that matters after RS-M7 — that the requests are intervals
# of ONE recording.  Its coverage (method / url / status / duration / label /
# span type / error status) is asserted against the real container by
# `tests/test_request_spans.php`.
test: build
...
# Serve test-programs/ under the recorder, with request spans going into the
# worker's own container.  `src/web_bootstrap.php` is the auto_prepend that
# labels each request (framework / route / error.message) through the
# extension's `codetracer_span_annotate()`; it replaced the pre-RS-M7
# `src/auto_prepend.php`, which wrote a JSONL sidecar and never touched the
# container.
demo: build
    CODETRACER_ENABLED=1 \
    LD_LIBRARY_PATH="${TRACE_FORMAT_NIM_DIR}:${LD_LIBRARY_PATH:-}" \
    php -d extension=ext/modules/codetracer.so \
        -d auto_prepend_file=src/web_bootstrap.php \
        -S localhost:8095 -t test-programs/
